...
export type FiMStoryChapter = (string | FiMHTMLTree)[]
export type FiMStoryChapters = { [key: string]: (string | FiMHTMLTree)[] }
export interface FiMStoryFull {
	Format: Exclude<FiMFormatType, FiMFormatType.RAW>
	Title: string
	Author: string
	Chapters: FiMStoryChapters
}
export interface FiMStoryRaw {
	Format: FiMFormatType.RAW
	Text: string[]
}
export type FiMStoryType = FiMStoryRaw | FiMStoryFull

export interface FiMHTMLTree {
	tag: string
	attr?: { [key: string]: any }
	children?: (string | FiMHTMLTree)[]
	data?: string
}
function objectify_html(o: HTMLElement, depth: number = 1): FiMHTMLTree | string {
	if (o.nodeType === 3) {
		if (o.textContent?.trim() !== o.textContent) return o.textContent!.replace(/^(\r\n)?                /g, '') // what the fuck
		return o.textContent!
	}

	const tree: FiMHTMLTree = { tag: o.tagName.toLowerCase() }
